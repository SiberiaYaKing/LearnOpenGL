...
//函数指针
//typedef void(*glGetivProc)(GLuint, GLenum, GLint*);
//typedef void(*glGetInfoLogProc)(GLuint, GLsizei, GLsizei*, GLchar*);
//void check_Compile(unsigned int&, glGetivProc, glGetInfoLogProc, GLenum);
...
void loadShader(unsigned int& shaderProgram) {
	...
	//check_Compile(vertexShader, glGetShaderiv, glGetShaderInfoLog, GL_COMPILE_STATUS);
	//check_Compile(fragmentShader, glGetShaderiv, glGetShaderInfoLog, GL_COMPILE_STATUS);
	//check_Compile(shaderProgram, glGetProgramiv, glGetProgramInfoLog, GL_LINK_STATUS);
}


//检查着色器编译状况
//void check_Compile(const unsigned int &ref_id, glGetivProc glGetiv, glGetInfoLogProc glGetInfoLog, GLenum e) {
//	int success;
//	char infoLog[512];
//	glGetiv(ref_id, e, &success); //获取编译状态
//	if (!success) {
//		glGetInfoLog(ref_id, 512, NULL, infoLog); //获取编译日志
//		cout << "ERROR::COMPILATION_FAILED\n" << infoLog << endl;
//	}
//}
